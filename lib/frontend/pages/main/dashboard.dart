...
class DashboardPageState extends State<DashboardPage>
                            ...
                            child: RichText(
                              text: TextSpan(
                                children: [
                                  TextSpan(
                                    text:
                                        isInitialized ? daysLeft.toString() : "?",
                                    style: TextStyle(
                                        color: isInitialized
                                            ? currentPhase.getColor()
                                            : Colors.grey,
                ...
                child: CycleWheelWidget(
                  onItemChanged: (phase, cycleDay, total, daysRemaining) {
                    setState(() {
                      ...
                      if (temperatureData.isNotEmpty &&
                          cycleDay > 0 &&
                          cycleDay <= temperatureData.length) {
                        selectedDayIndex = cycleDay - 1;
                        daysLeft = calculateDaysUntilPhaseChangeFromIndex(
                            selectedDayIndex);
                      }
                  ...
                  cycleWheelStateCallback: (state) {
                    ...
                    if (mounted && temperatureData.isNotEmpty) {
                      ...
                      int todayIndex = temperatureData
                          .indexWhere((element) => element.date.isSameDay(today));
                      ...
                      setState(() {
                        ...
                        if (cycleStartIndex != -1) {
                          ...
                          totalCycleDays = temperatureData.length > 28
                              ? 28
                              : temperatureData.length;
                        }
        
                        daysLeft =
                            calculateDaysUntilPhaseChangeFromIndex(todayIndex);
                        ...
                        if (_cycleWheelState != null) {
                          ...
                          if (todayIndex != -1) {
                            setState(() {
                              selectedDayIndex = todayIndex;
                              currentPhase =
                                  temperatureData[todayIndex].cyclePhase;
                              currentCycleDay = todayIndex + 1;
                              daysLeft = calculateDaysUntilPhaseChangeFromIndex(
                                  todayIndex);
                              _animationController.forward(from: 0.0);
  ...
  // Daten mit dem CyclePhasePredictor neu analysieren
  var analyzedData = cyclePhasePredictor.analyzeCurrentCycle(newData);
  
  // Zukünftige Phasen vorhersagen (3 Zyklen)
  temperatureData = cyclePhasePredictor.predictFutureCyclePhases(analyzedData, 3);
  
  if (mounted) {
    // Dashboard aktualisieren
    setState(() {
      // Den aktuellen Tag neu auswählen
      DateTime today = DateTime.now();
      int todayIndex = temperatureData.indexWhere(
          (element) => element.date.isSameDay(today));
  ...
  bool isCurrentDaySelected() {
    if (temperatureData.isEmpty ||
        selectedDayIndex < 0 ||
        selectedDayIndex >= temperatureData.length) {
      return false;
    }
  ...
  int findCycleStartIndex() {
    ...
    for (int i = 0; i < temperatureData.length; i++) {
      if (temperatureData[i].cyclePhase == CyclePhaseType.menstruation) {
        if (i == 0 ||
            temperatureData[i - 1].cyclePhase != CyclePhaseType.menstruation) {
          return i;
        }
      }
    }
  ...
  int findCycleLength(int cycleStartIndex) {
    ...
    for (int i = cycleStartIndex + 1; i < temperatureData.length; i++) {
      if (temperatureData[i].cyclePhase == CyclePhaseType.menstruation &&
          (i == 0 ||
              temperatureData[i - 1].cyclePhase !=
